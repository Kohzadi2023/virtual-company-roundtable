...
export function MeetingAgentRow({ data, onActivate, onToggleSkip, onSaveChat, onOpenChat }: MeetingAgentRowProps) {
  ...
  return (
    <div
      className={`grid grid-cols-1 gap-3 border-b border-slate-200 px-3 py-3 transition sm:px-4 lg:grid-cols-[minmax(250px,1.2fr)_minmax(220px,0.9fr)_minmax(340px,1.4fr)] lg:items-center lg:gap-4 ${data.active ? 'bg-emerald-50/60' : 'bg-white hover:bg-slate-50/70'}`}
    ...
    >
      ...
      <div className="min-w-0">
        <div className="mb-1.5 text-[9px] font-bold uppercase tracking-wide text-slate-400 lg:hidden">Meeting Status</div>
      ...
      <div className="min-w-0">
        <div className="mb-1.5 text-[9px] font-bold uppercase tracking-wide text-slate-400 lg:hidden">External Chat</div>
        {editState === 'removing' ? (
          ...
          <div className="space-y-1.5">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
              <input
                ...
                className="w-full min-w-0 flex-1 rounded-md border border-slate-300 bg-white px-2.5 py-1.5 text-xs text-slate-800 outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 disabled:bg-slate-50"
              />
