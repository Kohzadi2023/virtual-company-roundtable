...
import { MeetingDevOverrideDialog } from '@/components/MeetingDevOverrideDialog';
...
export function DevToolsMenu() {
  ...
  const [meetingOverrideOpen, setMeetingOverrideOpen] = useState(false);
  ...
  return (
    <>
      <details className="group relative">
        ...
        <div className="absolute bottom-8 start-0 z-[90] w-72 rounded-xl border border-amber-200 bg-white p-2 shadow-2xl">
          ...
          <button
            ...
            onClick={() => setMeetingOverrideOpen(true)}
            className="mt-1 flex w-full items-start gap-2 rounded-lg px-2.5 py-2 text-start text-xs text-slate-700 transition hover:bg-amber-50 hover:text-slate-950 disabled:cursor-not-allowed disabled:opacity-40"
          >
            <span className="mt-0.5" aria-hidden="true">🧪</span>
            <span>
              <span className="block font-bold">Meeting Round Override</span>
              <span className="mt-0.5 block leading-4 text-slate-500">Debug-only jump and rename controls removed from the normal meeting workflow.</span>
            </span>
          </button>
      ...
      {meetingOverrideOpen && activeRoomId ? <MeetingDevOverrideDialog roomId={activeRoomId} onClose={() => setMeetingOverrideOpen(false)} /> : null}
    </>
  );
}
