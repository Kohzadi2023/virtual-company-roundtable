    ...
    <>
      ...
      {open && (
        <>
              ...
              <aside className="flex w-72 shrink-0 flex-col border-e border-slate-200 bg-slate-50">
                <div className="border-b border-slate-200 px-4 py-4">
                  ...
                  <button
                    ...
                    className="mt-4 w-full rounded-lg bg-violet-700 px-3 py-2.5 text-xs font-bold text-white shadow-sm transition hover:bg-violet-800"
                  >
                    + New Project
                  </button>
                </div>
                    ...
                    {projects.map(project => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex w-full items-center gap-2 rounded-lg border px-3 py-2.5 text-start transition ${selected ? 'border-violet-300 bg-white shadow-sm' : 'border-transparent hover:border-slate-200 hover:bg-white'}`}
                        >
                        ...
                        {projectRooms.length === 0 ? (
                          ...
                          <div className="grid gap-3 lg:grid-cols-2">
                            {projectRooms.map(room => (
                              <div key={room.id} className={`rounded-xl border p-4 ${room.id === activeRoomId ? 'border-violet-300 bg-violet-50/40' : 'border-slate-200 bg-white'}`}>
          ...
          {newProjectOpen && (
            <div
              className="fixed inset-0 z-[140] grid place-items-center bg-slate-950/45 p-6 backdrop-blur-sm"
              role="dialog"
              aria-modal="true"
            ...
            >
              <form
                ...
                className="w-[min(560px,92vw)] rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl"
              >
                ...
                <label className="mt-5 block">
                  ...
                  <input
                    ...
                    placeholder="e.g. Offline Workout Tracking"
                    className="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none transition focus:border-violet-500 focus:ring-2 focus:ring-violet-100"
                  />
                </label>
                ...
                <label className="mt-4 block">
                  ...
                  <textarea
                    ...
                    rows={4}
                    className="w-full resize-y rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none transition focus:border-violet-500 focus:ring-2 focus:ring-violet-100"
                  />
                </label>

                <div className="mt-6 flex justify-end gap-2">
                  <button type="button" onClick={() => setNewProjectOpen(false)} className="rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-50">Cancel</button>
                  <button type="submit" disabled={!projectName.trim()} className="rounded-lg bg-violet-700 px-5 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-violet-800 disabled:cursor-not-allowed disabled:opacity-40">Create Project</button>
                </div>
              </form>
            </div>
          )}
